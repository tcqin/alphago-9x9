...
The final version of AlphaGo 9x9 combines CNNs and MCTS (Monte Carlo Tree Search). It was able to beat
nfBot, a GNU Go 3.8 engine running on [northflank.com](northflank.com).
...
- `models_filtered/rl_network`: highest version of `models_filtered/rl_network_v{i}.pth`
    - `v1`: used `value_network_sl` and TD advantages, won 145/200 games against the SL network (2026-02-27)
...
# 2026-02-28
Training 3 separate value network models:
- `value_network_sl`: Trained on just the SL vs SL self-play games
...
Value network on 500k SL games had a 0.566 std and 0.731 sign accuracy, with 0.421 above abs(0.5)
Value network on 1m SL games + 265k RL games had a 0.566 std and 0.739 sign accuracy, with 0.417 above abs(0.5)
